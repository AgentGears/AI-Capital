...
class ProgramRepository:
    ...
    def _migrate(self) -> None:
        ...
        if version != _SCHEMA_VERSION:
            raise IntegrityViolation(
                f"unsupported store schema version {version}"
            )
        ...
        *,
        ...
        with self._transaction():
            ...
            if exists is not None:
                raise PersistenceConflict(f"Program already exists: {program.program_id}")
    ...
    def list_events(self, program_id: str) -> tuple[Event, ...]:
        rows = self._connection.execute(
            "SELECT event_json FROM events WHERE program_id = ? ORDER BY sequence",
            (program_id,),
        ).fetchall()
        events: list[Event] = []
        for row in rows:
        ...
        for index, event in enumerate(events):
            ...
            if previous_revision is None:
                if candidate.revision != 0:
                    raise IntegrityViolation("created Program snapshot must be revision 0")
            elif candidate.revision != previous_revision + 1:
