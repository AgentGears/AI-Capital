...
class K8ReviewRound40Tests(unittest.TestCase):
    ...
    def test_v7_recall_index_migration_authenticates_event_body_before_rebuild(self):
        with tempfile.TemporaryDirectory() as directory:
            database = Path(directory) / "host.db"
            programs = ProgramRepository(database)
            program = programs.create(
                Program("p-1", 0, "legacy recall-index authentication")
            )
            ...
            with programs._transaction():
                ...
                programs._db.execute(
                    ...
                    "event_json, '$.payload.program.objective', 'corrupted objective'"
                    "), context_recall_invalidated = 0 WHERE event_id = ?",
                    (event_id,),
                )
                programs._db.execute(
                    "UPDATE component_schema SET version = 7 "
            ...
            try:
                with self.assertRaisesRegex(
                    IntegrityViolation,
                    "Context Event integrity mismatch",
                ):
                    ContextRepository(reopened)
            finally:
                reopened.close()

    def test_v8_migration_recovers_compiled_invalidation_after_both_projections_lost(self):
        with tempfile.TemporaryDirectory() as directory:
            database = Path(directory) / "host.db"
            programs = ProgramRepository(database)
            program = programs.create(
                Program("p-1", 0, "legacy compiled envelope recovery")
            )
    ...
    def test_v8_migration_fails_closed_when_invalidated_event_cannot_be_classified(self):
        with tempfile.TemporaryDirectory() as directory:
            database = Path(directory) / "host.db"
            programs = ProgramRepository(database)
            program = programs.create(
                Program("p-1", 0, "legacy invalidation ambiguity")
            )
            ContextRepository(programs)
            event_id = self._program_event_id(programs, program.program_id)
            ...
            with programs._transaction():
                ...
                programs._db.execute(
                    "UPDATE events SET event_type = 'program.invalidated', "
                    "context_recall_invalidated = 1 WHERE event_id = ?",
                    (event_id,),
                )
                programs._db.execute(
