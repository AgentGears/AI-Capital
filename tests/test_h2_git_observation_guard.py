...
class H2GitObservationGuardTests(unittest.TestCase):
    ...
    def test_implicit_helper_and_lazy_fetch_configuration_is_rejected(self):
        configs = (
            "[alias]\n\tinspect = status\n",
            "[pager]\n\tstatus = cat\n",
            "[core]\n\tpager = cat\n",
            "[core]\n\talternateRefsCommand = git for-each-ref\n",
            "[extensions]\n\tpartialClone = origin\n",
            "[remote \"origin\"]\n\tpromisor = true\n",
    ...
    def test_http_alternates_metadata_is_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            repository = self._repository(
                root,
            ...
            (info / "http-alternates").write_text("https://example.invalid/objects\n")
            with self.assertRaises(InvalidRequest):
                validate_git_repository(repository)
