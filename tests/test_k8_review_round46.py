...
class K8ReviewRound46Tests(unittest.TestCase):
    @staticmethod
    ...
    def _coherently_mutate_persisted_source(
        ...
        event = K8ReviewRound46Tests._event(programs, event_id)
    ...
    def test_invalidated_optional_persisted_source_fails_projection_rebuild_on_restart(self):
        for priority in (
            ...
            with self.subTest(priority=priority.value), tempfile.TemporaryDirectory() as directory:
                ...
                program = programs.create(
                    Program("p-1", 0, f"Round 46 restart {priority.value}")
                )
    ...
    def test_invalidated_optional_persisted_source_fails_hot_preflight_before_compile(self):
        for priority in (
            ContextPriority.RECENT_INTERACTION,
            ContextPriority.ADVISORY_MEMORY,
        ):
            with self.subTest(priority=priority.value), tempfile.TemporaryDirectory() as directory:
                ...
                try:
                    program = programs.create(
                        Program("p-1", 0, f"Round 46 hot preflight {priority.value}")
                    )
                    contexts = ContextRepository(programs)
                    source_ref = contexts.persist_source(
                        program.program_id,
